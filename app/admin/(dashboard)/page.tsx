...
export default async function AdminDashboardPage() {
  const { now, since30 } = getReportWindow();
  const { recent: orders, pendingCount, activeBrands, categoriesByRevenue } = await adminOrderRepository.dashboard();
  const last30 = orders.filter((o) => o.createdAt.toISOString() >= since30);
  const prev30 = orders.filter((o) => o.createdAt.toISOString() < since30);
  const revenue30 = last30.reduce((sum, o) => sum + o.totalMinor / 100, 0);
  const revenuePrev = prev30.reduce((sum, o) => sum + o.totalMinor / 100, 0);
  ...
  for (const o of orders) {
    const key = dayKey(o.createdAt.toISOString());
    revenueByDay.set(key, (revenueByDay.get(key) ?? 0) + o.totalMinor / 100);
  ...
  const weeks: { label: string; total: number }[] = Array.from({ length: 7 }, (_, i) => {
    ...
    const total = orders
      ...
      .reduce((sum, o) => sum + o.totalMinor / 100, 0);
    return { label: `Wk ${i + 1}`, total };
  });

  const totalCategoryRevenue = categoriesByRevenue.reduce((sum, row) => sum + Number(row.revenueMinor), 0);
  const categorySplit = categoriesByRevenue
    .map(({ name, revenueMinor }) => ({
      name,
      pct: totalCategoryRevenue > 0 ? Math.round((Number(revenueMinor) / totalCategoryRevenue) * 100) : 0,
    }))
        ...
        <div className="mt-6 rounded-card border border-border bg-surface p-4.5 shadow-sm sm:p-6.5">
          ...
          <div className="flex flex-col divide-y divide-border sm:hidden">
            {recentOrders.map((o) => (
              ...
              >
                <div
                  className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full font-sans text-[11px] font-extrabold ${avatarClass(o.customerName)}`}
                >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate font-sans text-xs text-text-muted">{o.customerName}</p>
                </div>
                <div className="shrink-0 text-right">
                  <p className="font-sans text-sm font-semibold text-text-primary">
                    {formatNaira(o.totalMinor / 100)}
                  </p>
                  <div className="mt-1">
                  ...
                  <tr key={o.id} className="[&>td]:border-b [&>td]:border-border last:[&>td]:border-none">
                    ...
                    <td className="py-4 pr-3.5">
                      <div className="flex items-center gap-2.5">
                        <div
                          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full font-sans text-[11px] font-extrabold ${avatarClass(o.customerName)}`}
                        >
                          {initials(o.customerName)}
                        </div>
                        <p className="font-sans text-sm text-text-primary">{o.customerName}</p>
                      </div>
                    </td>
